          ...
          <Card className="mb-3">
            ...
            <div class="row">
              <div class="column-btns column-left">
                  <Button color="primary btn-barra-simular" onClick={this.toggle('nested_parent')}>
                    SIMULACIÓN MASIVA
                    </Button>
            </div>
                ...
                <Tab label="Diesel" >
                    <Table responsive>
                      <thead>
                          <tr>
                            ...
                            { viewprice ?
                            this.state.simular ? <th className="header-table min-font">SIMULACIÓN DIARIA</th> : ''
                            :
                            this.state.simular ? <th className="header-table min-font">SIMULACIÓN FRANJA 1</th> : '' }
                            {viewprice ?  <th className="header-table min-font">PRECIO RECOMENDADO </th> : <th className="header-table min-font"><span className="meaning">PRECIO RECOMENDADO FRANJA 1 </span><span className="detail">Precio recomendado desde la hora { "00:01" } hasta la hora { "11:00" }</span> </th> }
                            { viewprice ? 
                             ''
                            :
                            this.state.simular ? <th className="header-table min-font">SIMULACIÓN FRANJA 2</th> : '' }
                            {viewprice ? '' : <th className="header-table min-font"><span className="meaning">PRECIO RECOMENDADO FRANJA 2 </span><span className="detail">Precio recomendado desde la hora { "11:01" } hasta la hora { "18:00" }</span> </th> }
                            { viewprice ? 
                            ''
                            :
                            this.state.simular ? <th className="header-table min-font">SIMULACIÓN FRANJA 3</th> : '' }
                            {viewprice ? '' : <th className="header-table min-font"><span className="meaning">PRECIO RECOMENDADO FRANJA 3 </span><span className="detail">Precio recomendado desde la hora { "18:01" } hasta la hora { "24:00" }</span> </th> }
                    ...
                    <tbody>
                    {
                      ...
                      dataReal[0].map((prop,i) => {
                          ...
                          { viewprice ?
                            <td className="text-center text-shadow text-shadow">{ this.state.simularid === i  ? <input type="radio" name={ 'franja1' + producto[0].nombre + i } value={producto[0].simularfranja1}  /> :''} $ {producto[0].pvprecomendado } </td> :
...
<Row>
        ...
        <Row>
          ...
          <Col xl={4} lg={12} md={12}>
            <Card>
              <CardHeader>MARGEN TEÓRICO</CardHeader>
              <CardBody>
